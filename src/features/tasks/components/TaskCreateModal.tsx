...
function useProjectSuggestions(q: string) {
  const query = (q || '').trim()
...
function useAssigneeSuggestions(q: string, projectId: Id | '' = '') {
  const query = (q || '').trim()
  const isAuto = query.length >= 2
...
export default function TaskCreateModal({
  ...
  const [form, setForm] = useState<TaskCreateForm>(EMPTY)
  ...
  const set = <K extends keyof TaskCreateForm>(key: K, val: TaskCreateForm[K]) =>
      ...
      <div className="relative bg-bg-surface border border-border rounded-2xl shadow-deep w-full max-w-[520px] animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          ...
          <input
            ...
            onKeyDown={(e: ReactKeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter' && !e.shiftKey) handleSubmit() }}
