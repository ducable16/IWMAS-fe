...
type UserFilterKey = 'assigneeId' | 'reporterId'
...
function ToggleChip({ active, onClick, children, colorCls = '' }: ToggleChipProps) {
  return (
    <button
      onClick={onClick}
      className={clsx(
 ...
 */
function UserSearchSelect({ filterKey, selectedId, users, onChange }: UserSearchSelectProps) {
  const [query, setQuery] = useState('')
  const [open, setOpen] = useState(false)
  const wrapRef = useRef<HTMLDivElement | null>(null)

  const selected = users.find((u) => u.id === selectedId) || null
  const selectedName = selected ? (selected.fullName || '?') : null
 ...
 */
function ProjectSearchSelect({ filterKey, selectedId, projects, onChange }: ProjectSearchSelectProps) {
  const [query, setQuery] = useState('')
  const [open, setOpen] = useState(false)
  const wrapRef = useRef<HTMLDivElement | null>(null)
...
export default function TaskFilterDrawer({
  ...
  const toggle = (key: MultiFilterKey, val: string) => {
