...
const STATUS_META_BY_KEY = TASK_STATUS_META as Record<string, { label: string; color?: string }>

function toDateKey(date: Date) {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`
}

function CalendarDay({ date, tasks, currentMonth, navigate }: CalendarDayProps) {
    ...
    >
      ...
      <div className="space-y-0.5">
        {visible.map((task) => {
          ...
          const pillCls = STATUS_META_BY_KEY[statusKey]?.color || 'bg-bg-hover text-text-secondary'
          return (
            <div
              key={task.id}
...
export default function TaskCalendarView({ filters }: { filters: TaskFilters }) {
