...
export function PriorityField({
  ...
  return (
    <div className="relative min-w-0 max-w-full" ref={activeDropdownRef(editingField === 'priority', dropdownRef)}>
      <button
        type="button"
        onClick={canEdit
          ...
          : undefined}
        className={clsx(
          'flex min-w-0 items-center gap-1.5 font-medium rounded-md px-1.5 py-0.5 -ml-1.5 transition-colors w-full text-left',
          canEdit ? 'hover:bg-bg-hover cursor-pointer' : 'cursor-default',
        )}
      >
        <span className={clsx('min-w-0 truncate', priorityMeta.cls)}>{priorityMeta.label}</span>
      </button>
