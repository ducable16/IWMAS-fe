...
export function ToolbarBtn({ icon: Icon, label, onMouseDown, active, disabled, children }) {
  return (
    <button
      type="button"
      title={label}
...
export function Dropdown({ triggerEl, children }) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      <div onMouseDown={e => { e.preventDefault(); setOpen(v => !v) }}>{triggerEl}</div>
      {open && (
        <div
...
export function TextTypeDropdown({ focusedBlock, onChangeType }) {
...
/* ─── List dropdown (≡) ───────────────────────────────────────────────────── */
export function ListDropdown({ onChangeType }) {
  return (
    <Dropdown
      triggerEl={
        <button type="button" className="flex items-center gap-0.5 px-1.5 py-1 rounded text-[12px] text-text-secondary hover:bg-bg-hover transition-colors select-none">
