...
const fmtFull   = (d) => dayjs(d).format('MMM D, YYYY')

// ─── Header unit builders ──────────────────────────────────────────────────────

/** Upper row: months (for weeks/months scale) or quarters */
function getUpperUnits(days, ppd, scale) {
  const units = []
  let cur = null
  days.forEach((d, i) => {
...
function getLowerUnits(days, ppd, scale) {
  if (scale === 'weeks') {
    return days.map((d, i) => ({
      key:   i,
      label: String(d.date()),
...
function BarTooltip({ task, startX, endX, centerY }) {
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function TaskTimelineView({ filters }) {
  const navigate     = useNavigate()
  const scrollRef    = useRef(null)
  const queryClient  = useQueryClient()
  const [scale, setScale]           = useState('weeks')
  const [tooltip, setTooltip]       = useState(null)        // { task, startX, endX, centerY }
  const [dragOverride, setDragOverride] = useState(null)    // { taskId, startDate, dueDate }

  // Refs used inside window listeners (avoid stale closures)
  const dragRef  = useRef(null)   // { taskId, handle, startClientX, origStart, origDue, hasDragged }
  ...
  const getBarViewportX = useCallback((barLeft, barWidth) => {
        ...
        <div style={{ minWidth: LEFT_W + timelineW }}>
          ...
          {rawTasks.length === 0 ? (
            ...
            rawTasks.map(task => {
              ...
              const prio   = PRIORITY_META[task.priority] || { dot: 'bg-border-strong' }
              const status = TASK_STATUS_META[task.status] || { label: task.status, color: 'bg-bg-hover text-text-secondary' }
              const color  = BAR_COLOR[task.status]        || BAR_COLOR.TODO
                      ...
                      return (
                        <div
                          ...
                          onMouseEnter={() => {
                            ...
                            const rowEl = scrollRef.current?.querySelector(`[data-row-id="${task.id}"]`)
                            const centerY = rowEl
