...
import type { ApiError, Id, MemberView, PageResponse, Project, User } from '@/types'
...
type ProjectFormErrors = Partial<Record<keyof ProjectFormState, string | null>>
...
type ManagerAutocompleteParams = {
...
function useManagerAutocomplete(query: string, params: ManagerAutocompleteParams = {}) {
...
export default function ProjectFormModal({ open, project, onClose }: ProjectFormModalProps) {
  ...
  const [form, setForm] = useState<ProjectFormState>(BLANK)
  ...
  const set = (key: keyof ProjectFormState) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
  ...
  const validate = () => {
    ...
    if (!isEdit) {
      if (
        ...
        Number.isNaN(Number(form.managerEffortPercent)) ||
      ...
      <div className="relative w-full max-w-[500px] bg-bg-surface border border-border rounded-xl shadow-card overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          ...
          <AutocompleteSelect
            ...
            renderOption={(item) => (
              <ManagerSuggestion item={item as ManagerSuggestionItem} startDate={form.startDate} endDate={form.endDate} />
            )}
          />
