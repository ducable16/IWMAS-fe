...
import { useSprintSummary } from '../hooks/useDashboard'
...
export default function SprintSummary() {
  const { data: sprints, isLoading } = useSprintSummary()
  const list = sprints || []
  ...
  return (
    <div className="card p-5">
      ...
      {isLoading ? (
        <p className="text-[12.5px] text-text-muted py-2">Loading…</p>
      ) : list.length === 0 ? (
        <p className="text-[12.5px] text-text-muted py-2">No sprints.</p>
      ) : (
            ...
            <div key={sprint.name} className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className={clsx(RISK_BADGE[sprint.risk] || 'badge-neutral', 'capitalize')}>
                    {sprint.risk}
                  </span>
                </div>
