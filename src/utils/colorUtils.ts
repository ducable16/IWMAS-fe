// Inline colour tokens — keep in sync with tailwind.config.ts (Notion palette)
// Used where Tailwind classes can't reach (chart fills, inline svg, etc.)

export const workloadColor = (score: number): string =>
...
export const priorityColor = (p: unknown): string =>
  ({
    critical: '#B43A18',
    high:     '#DD5B00',
    medium:   '#DD5B00',
...
export const statusColor = (s: unknown): string =>
  ({
    done:        '#1AAE39',
    in_progress: '#0075DE',
    review:      '#391C57',
