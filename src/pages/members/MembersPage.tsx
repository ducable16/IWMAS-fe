...
import {
  UserPlus, Search, ChevronDown, ChevronLeft, ChevronRight,
...
const DEFAULT_PARAMS: MemberFilterParams = {
  search:        '',
  role:          '',
  active:        '',   // '' | 'true' | 'false'
  sortBy:        'fullName',
...
export default function MembersPage() {
  ...
  // ── Param helpers ──
  const set = useCallback(<K extends keyof MemberFilterParams>(key: K, val: MemberFilterParams[K]) => {
    setParams((prev) => ({
      ...prev,
      [key]: val,
  ...
  useEffect(() => {
    ...
    const match = members.find((u) => String(u.id) === deepUserId)
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        {/* Role filter */}
        <FilterSelect value={params.role} onChange={(v) => set('role', v as MemberFilterParams['role'])} options={ROLE_OPTIONS} />

        {/* Status filter */}
        <FilterSelect value={params.active} onChange={(v) => set('active', v as MemberFilterParams['active'])} options={STATUS_OPTIONS} />
