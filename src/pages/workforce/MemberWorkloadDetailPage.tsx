...
import type { Id, ProjectWorkloadAllocation, WorkloadMember, WorkloadTask } from '@/types'
...
function ProjectAllocationsTable({
  ...
  allocations?: Array<ProjectWorkloadAllocation & {
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-[18px] font-bold bg-gradient-to-br from-accent/20 to-accent/5 border border-accent/15 text-accent shrink-0">
              {(data.userFullName || data.fullName)?.split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase() || '?'}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-[18px] font-semibold text-text-primary">{data.userFullName || data.fullName}</h2>
                {isSelf && (
...
function SelfWorkloadView({
  ...
  return <WorkloadDetailContent data={data as WorkloadMember} weekStart={weekStart} weekEnd={weekEnd} onWeekChange={onWeekChange} isSelf />
}
...
function OtherUserWorkloadView({
  ...
  return <WorkloadDetailContent data={data as WorkloadMember} weekStart={weekStart} weekEnd={weekEnd} onWeekChange={onWeekChange} isSelf={false} />
}
