...
import type { Id, TaskComment, UpdateTaskRequest } from '@/types'

type AvatarSize = 'xs' | 'sm' | 'md'
type EditableField = 'assignee' | 'priority' | 'type' | 'startDate' | 'dueDate' | 'estimate' | 'labels'
type MentionMap = Record<string, Id>
...
const TASK_TYPE_LABEL_BY_KEY = TASK_TYPE_LABEL as Record<string, string>
...
function StatusDropdown({ current, taskId, canChange = true }: StatusDropdownProps) {
...
function HistoryTab({ taskId }: HistoryTabProps) {
...
function CommentItem({ comment, taskId, currentUserId, mentionMap }: CommentItemProps) {
...
function CommentsTab({ taskId, comments = [], projectId }: CommentsTabProps) {
...
export default function TaskDetailPage() {
  ...
  const priorityM = PRIORITY_META[priority as keyof typeof PRIORITY_META] || PRIORITY_META.MEDIUM
    ...
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="min-h-[80px]">
              {activeTab === 'comments' && <CommentsTab taskId={task.id} comments={task.comments} projectId={task.projectId} />}
        ...
        <div className="w-[280px] shrink-0 space-y-4 sticky top-[68px]">
          ...
          <StatusDropdown current={status} taskId={task.id} canChange={canEditTask} />
          ...
          <div className="card p-4 space-y-0">
            ...
            <DetailRow icon={GitBranch} label="Type">
              <div className="relative" ref={editingField === 'type' ? dropdownRef : null}>
                ...
                >
                  {TASK_TYPE_LABEL_BY_KEY[task?.type ?? ''] || task?.type || '—'}
                </button>
