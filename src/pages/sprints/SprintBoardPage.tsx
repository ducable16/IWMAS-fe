...
const EMPTY_COLUMNS: SprintBoardColumns = {
...
export default function SprintBoardPage() {
  ...
  const [columns, setColumns] = useState<SprintBoardColumns>(board || EMPTY_COLUMNS)
  const [dragging, setDragging] = useState<DraggingTask | null>(null)
  const [dragOver, setDragOver] = useState<SprintBoardColumnId | null>(null)

  useEffect(() => {
    if (board) setColumns(board)
  }, [board])

  const handleDragStart = (taskId: string, fromCol: SprintBoardColumnId) => {
    setDragging({ taskId, fromCol })
  }

  const handleDrop = (toCol: SprintBoardColumnId) => {
    ...
    setColumns((prev) => {
      ...
      const task = from.tasks.splice(taskIdx, 1)[0]
      if (!task) return prev
      to.tasks = [{ ...task, done: toCol === 'done' }, ...to.tasks]
